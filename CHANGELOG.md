...
All notable repository changes are recorded here. This file tracks changes to the content package and its publishing support files; it is not reader-facing article copy.
...
Add an entry when a change affects the article’s factual scope, search or social presentation, content workflow, published assets, or release behavior. Small typo corrections do not require an entry unless they change a technical meaning, date, number, model name, or citation. Keep each entry factual, concise, and tied to a reviewable pull request or release record.
